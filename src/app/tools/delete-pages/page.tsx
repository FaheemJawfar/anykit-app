...
export default function DeletePages() {
  ...
  const removePages = async () => {
    if (!file) return;
    const toDelete = parsePages(pagesToDelete);
    if (toDelete.length === 0) {
      setError("Please enter at least one page number.");
      return;
    }
    const invalid = toDelete.filter((p) => p < 1 || p > pageCount);
    if (invalid.length > 0) {
    ...
    try {
      ...
      const doc = await PDFDocument.load(buffer);
      const indicesToDelete = new Set(toDelete.map((p) => p - 1));
      const indicesToKeep = doc.getPageIndices().filter((i) => !indicesToDelete.has(i));

      const newDoc = await PDFDocument.create();
      const pages = await newDoc.copyPages(doc, indicesToKeep);
      ...
      a.href = url;
      a.download = `removed-${toDelete.length}-pages.pdf`;
      a.click();
  ...
  const parsed = parsePages(pagesToDelete);
          ...
          {file ? (
            <Card className="border-border/40 shadow-xl shadow-primary/5 bg-card/40 backdrop-blur-sm rounded-[2.5rem] overflow-hidden">
              <div className="px-8 py-4 border-b border-border/40 bg-muted/30">
                <span className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Pages to Delete</span>
              </div>
              <CardContent className="p-8 space-y-8">
                ...
                {pagesToDelete && (
                  <div className="flex flex-wrap gap-2">
                    {parsed.map((p) => (
                      ...
                      >
                        {p}
                        {p >= 1 && p <= pageCount && <X className="w-3 h-3" />}
                      </span>
                    ))}
                  </div>
                )}
                ...
                <div className="p-4 rounded-2xl bg-muted/30 space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">Valid pages selected</span>
                    <span className="font-bold text-destructive">{validCount}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">Remaining pages</span>
                    <span className="font-bold text-primary">{pageCount - validCount}</span>
                  </div>
                </div>
                ...
                >
                  {loading ? (
                    ...
                    <>
                      <Trash2 className="w-5 h-5 mr-2" />
                      Delete {validCount} Page{validCount !== 1 ? "s" : ""}
                    </>
                  )}
                </Button>
              </CardContent>
            ...
            <div className="h-full flex flex-col items-center justify-center p-12 text-center space-y-6 bg-muted/20 rounded-[2.5rem] border border-dashed border-border/40 min-h-[400px]">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground max-w-xs mx-auto">
                  Upload a PDF file to remove specific pages from it.
                </p>
              </div>
            </div>
          )}
