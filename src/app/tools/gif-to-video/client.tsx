...
export default function GifToVideoClient() {
  ...
  const qualities = [
    { value: "low", label: "Low", desc: "Fast encoding" },
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      if (format === "mp4") {
        ...
        args.push("-c:v", "libvpx-vp9", "-crf", crf, "-b:v", "0", "-cpu-used", "4");
      }
        ...
        <div className="lg:col-span-4 order-last space-y-6">
                ...
                <div className="space-y-2">
                  {qualities.map((q) => (
                    ...
                    >
                      ...
                      <span className="text-[10px] opacity-70 font-bold">{q.desc}</span>
                    </button>
                  ))}
                </div>
              </div>
          ...
          <div className="p-6 rounded-3xl bg-primary/5 border border-primary/10 space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-bold text-xs uppercase tracking-wider text-primary">Why Convert?</h3>
            </div>
            <p className="text-[11px] text-muted-foreground leading-relaxed">
              MP4/WebM files are typically 5-10x smaller than GIFs and load faster on the web. Video formats also support higher color depth.
            </p>
          </div>
        </div>
          ...
          {!file && !result && (
            ...
            >
              ...
              <div className="text-center space-y-2">
                <p className="font-bold text-lg">Drop GIF file here</p>
          ...
          {processing && (
            <Card className="border-border/40 shadow-xl shadow-primary/5 bg-card/40 backdrop-blur-sm rounded-[2.5rem] overflow-hidden">
              <CardContent className="p-12 flex flex-col items-center gap-6">
                ...
                <div className="text-center space-y-2">
                  <p className="font-bold text-lg">Converting GIF...</p>
              ...
              <CardContent className="p-10 flex flex-col items-center gap-8">
                ...
                <div className="text-center space-y-1">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {formatFileSize(result.originalSize)} → {formatFileSize(result.outputSize)} · {format.toUpperCase()} · {result.time.toFixed(1)}s
                  </p>
                </div>
                <div className="flex gap-3 w-full max-w-md">
                  <Button
                    onClick={() => {
                      ...
                      a.href = result.url;
                      const base = file?.name.split(".")[0] || "gif";
