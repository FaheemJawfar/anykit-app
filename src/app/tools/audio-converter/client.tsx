...
const qualities = [
  { value: "320k", label: "Pro", desc: "320 kbps — Best" },
  { value: "192k", label: "Studio", desc: "192 kbps — Standard" },
  { value: "128k", label: "Light", desc: "128 kbps — Mobile" },
];
...
export default function AudioConverter() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      // For lossless formats, ignore bitrate override
      if (format === "wav" || format === "flac") {
        args.splice(3, args.length - 3, ...codecArgs);
      } else if (format === "ogg") {
        args[3] = "-q:a";
        args[4] = "6";
      }
        ...
        <div className="lg:col-span-4 order-last space-y-6">
          <Card className="border-border/40 shadow-xl shadow-primary/5 bg-card/40 backdrop-blur-sm rounded-[2.5rem] overflow-hidden">
            <CardContent className="p-8 space-y-8">
              ...
              {(format === "mp3" || format === "aac" || format === "ogg") && (
          ...
          <div className="p-6 rounded-3xl bg-primary/5 border border-primary/10 space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-bold text-xs uppercase tracking-wider text-primary">Codec Info</h3>
            </div>
            <p className="text-[11px] text-muted-foreground leading-relaxed">
              FLAC and WAV are lossless. Converting from MP3 to FLAC will not improve quality, but preserves it for editing.
            </p>
          </div>
        </div>
              ...
              <CardContent className="p-12 flex flex-col items-center gap-6">
                ...
                <div className="text-center space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">
                    To {format.toUpperCase()} · {progress}%
                  </p>
                </div>
            ...
            <Card className="border-emerald-500/30 shadow-2xl shadow-emerald-500/10 bg-gradient-to-br from-emerald-500/10 to-primary/5 backdrop-blur-sm rounded-[2.5rem] overflow-hidden">
              <CardContent className="p-10 flex flex-col items-center gap-8">
                ...
                <div className="text-center space-y-1">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {file?.name} → {format.toUpperCase()} · {result.time.toFixed(1)}s
                  </p>
                </div>
