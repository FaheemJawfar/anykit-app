...
import { Table as TableIcon, Copy, CheckCircle2, Plus, Minus, Trash2, Zap, LayoutTemplate } from "lucide-react";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        <div className="lg:col-span-8 space-y-6">
          <Card className="border-border/40 shadow-xl shadow-primary/5 bg-card/40 backdrop-blur-sm rounded-[2.5rem] overflow-hidden">
            <div className="px-8 py-6 border-b border-border/40 bg-muted/30 flex items-center gap-3"><TableIcon className="w-4 h-4 text-primary" /><span className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Table Editor ({rows}x{cols})</span></div>
            <CardContent className="p-8 overflow-auto">
              <table className="w-full border-collapse">
                <thead><tr>{headers.map((h, i) => (<th key={i} className="p-2 border border-border bg-muted/30"><div className="flex items-center gap-2"><input type="text" value={h} onChange={(e) => updateHeader(i, e.target.value)} placeholder={`Col ${i + 1}`} className="w-full px-2 py-1 bg-transparent border-none text-sm font-bold text-foreground focus:outline-none" /><button onClick={() => toggleAlignment(i)} className="text-[10px] text-muted-foreground uppercase font-black">{alignments[i][0]}</button></div></th>))}</tr></thead>
        ...
        <div className="lg:col-span-4 space-y-6 sticky top-24">
          ...
          <Card className="border-border/40 shadow-xl shadow-primary/5 bg-card/30 backdrop-blur-sm rounded-3xl overflow-hidden">
            <div className="px-6 py-4 border-b border-border/40 bg-muted/30 flex items-center gap-2"><LayoutTemplate className="w-4 h-4 text-primary" /><span className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Table Size</span></div>
            <CardContent className="p-6 space-y-2">
              <Button onClick={() => updateTableSize(rows + 1, cols)} variant="outline" className="w-full h-12 rounded-xl border-border/50 font-bold text-muted-foreground justify-start px-4"><Plus className="w-4 h-4 mr-2" /> Add Row</Button>
              <Button onClick={() => rows > 1 && updateTableSize(rows - 1, cols)} variant="outline" className="w-full h-12 rounded-xl border-border/50 font-bold text-muted-foreground justify-start px-4"><Minus className="w-4 h-4 mr-2" /> Remove Row</Button>
              <Button onClick={() => updateTableSize(rows, cols + 1)} variant="outline" className="w-full h-12 rounded-xl border-border/50 font-bold text-muted-foreground justify-start px-4"><Plus className="w-4 h-4 mr-2" /> Add Column</Button>
              <Button onClick={() => cols > 1 && updateTableSize(rows, cols - 1)} variant="outline" className="w-full h-12 rounded-xl border-border/50 font-bold text-muted-foreground justify-start px-4"><Minus className="w-4 h-4 mr-2" /> Remove Column</Button>
            </CardContent>
          </Card>
          <Card className="border-border/40 shadow-xl shadow-primary/5 bg-card/30 backdrop-blur-sm rounded-3xl overflow-hidden">
            <div className="px-6 py-4 border-b border-border/40 bg-muted/30 flex items-center gap-2"><TableIcon className="w-4 h-4 text-primary" /><span className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Generated Markdown</span></div>
            <CardContent className="p-6"><pre className="p-4 bg-muted/30 rounded-xl text-xs font-mono text-foreground overflow-auto border border-border">{markdownOutput}</pre></CardContent>
          </Card>
        </div>
      </div>
    </ToolLayout>
