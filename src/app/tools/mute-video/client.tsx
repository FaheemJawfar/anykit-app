...
export default function MuteVideoClient() {
  ...
  const [keepVideoCodec, setKeepVideoCodec] = useState(true);
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      const outputName = "muted" + inputExt;
        ...
        <div className="lg:col-span-4 order-last space-y-6">
            ...
            <CardContent className="p-8 space-y-8">
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <button
                    onClick={() => setKeepVideoCodec(true)}
                    className={cn(
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-black">Copy stream</span>
                    </div>
                    <p className="text-[11px] opacity-70 leading-tight mt-1">
                      Fastest — no re-encoding, no quality loss
                    </p>
                  </button>
                  <button
                    onClick={() => setKeepVideoCodec(false)}
                    className={cn(
                      "w-full p-3 rounded-xl border-2 transition-all text-left",
                      !keepVideoCodec
                        ? "bg-primary border-primary text-primary-foreground shadow-md"
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-black">Re-encode H.264</span>
                    </div>
                    <p className="text-[11px] opacity-70 leading-tight mt-1">
                      Better compatibility for some players
                    </p>
                  </button>
                </div>
              </div>
          ...
          <div className="p-6 rounded-3xl bg-primary/5 border border-primary/10 space-y-4">
            ...
            <p className="text-[11px] text-muted-foreground leading-relaxed">
              Strips the audio stream using the -an flag. With copy mode, only the container is rewritten — the video data is untouched.
            </p>
          </div>
        </div>
          ...
          {processing && (
            <Card className="border-border/40 shadow-xl shadow-primary/5 bg-card/40 backdrop-blur-sm rounded-[2.5rem] overflow-hidden">
              <CardContent className="p-12 flex flex-col items-center gap-6">
                ...
                <div className="text-center space-y-2">
                  <p className="font-bold text-lg">Removing audio...</p>
            ...
            <Card className="border-emerald-500/30 shadow-2xl shadow-emerald-500/10 bg-gradient-to-br from-emerald-500/10 to-primary/5 backdrop-blur-sm rounded-[2.5rem] overflow-hidden">
              <CardContent className="p-10 flex flex-col items-center gap-8">
                ...
                <div className="text-center space-y-1">
                  <p className="text-2xl font-black">Muted!</p>
                  <p className="text-sm text-muted-foreground">
                    Audio removed · {keepVideoCodec ? "Stream copied" : "Re-encoded"} · {result.time.toFixed(1)}s
                  </p>
                </div>
