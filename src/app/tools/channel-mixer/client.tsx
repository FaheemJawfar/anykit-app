...
type ChannelMode = "stereo" | "mono" | "left" | "right";
...
export default function ChannelMixer() {
  ...
  const getFilter = (m: ChannelMode): string => {
    switch (m) {
      case "mono":
      ...
      case "stereo":
        return "aecho=0.8:0.9:1:0.3"; // subtle stereo widening as default
      default:
        return "pan=mono|c0=0.5*c0+0.5*c1";
    }
  };
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      const outputName = "mixed" + ext;

      await ffmpeg.writeFile(inputName, await fetchFile(file));

      const filter = getFilter(mode);
  ...
  const modes: { value: ChannelMode; label: string; desc: string; icon: React.ElementType }[] = [
    { value: "mono", label: "Mono", desc: "Mix both channels", icon: Speaker },
    { value: "stereo", label: "Stereo", desc: "Keep/Enhance stereo", icon: Headphones },
    { value: "left", label: "Left Only", desc: "Use left channel", icon: Speaker },
    { value: "right", label: "Right Only", desc: "Use right channel", icon: Speaker },
  ];
        ...
        <div className="lg:col-span-4 order-last space-y-6">
          ...
          <div className="p-6 rounded-3xl bg-primary/5 border border-primary/10 space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-bold text-xs uppercase tracking-wider text-primary">Channel Mixing</h3>
            </div>
            <p className="text-[11px] text-muted-foreground leading-relaxed">
              Convert stereo to mono by averaging both channels, or isolate a single channel for focused listening.
            </p>
          </div>
        </div>
          ...
          {processing && (
            <Card className="border-border/40 shadow-xl shadow-primary/5 bg-card/40 backdrop-blur-sm rounded-[2.5rem] overflow-hidden">
              <CardContent className="p-12 flex flex-col items-center gap-6">
                ...
                <div className="text-center space-y-2">
                  <p className="font-bold text-lg">Mixing channels...</p>
            ...
            <Card className="border-emerald-500/30 shadow-2xl shadow-emerald-500/10 bg-gradient-to-br from-emerald-500/10 to-primary/5 backdrop-blur-sm rounded-[2.5rem] overflow-hidden">
              <CardContent className="p-10 flex flex-col items-center gap-8">
                ...
                <div className="text-center space-y-1">
                  <p className="text-2xl font-black">Mixed!</p>
                  <p className="text-sm text-muted-foreground">
                    {mode.charAt(0).toUpperCase() + mode.slice(1)} mode · {result.time.toFixed(1)}s
                  </p>
                </div>
